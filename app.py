...
# ============ تنظیمات شما ============
TELEGRAM_BOT_TOKEN = os.environ.get('TELEGRAM_TOKEN', '8495725535:AAFEfTgqtnB1F5Qn5vdreDd6Z6JpTBDaHKg')
GEMINI_API_KEY = os.environ.get('GEMINI_KEY', 'AIzaSyC2DlIw2gf2hXbg07IY_4T1PSQ0SVFjkHc')
# ====================================
...
def format_data_for_display(df, sections):
    ...
    for i, section in enumerate(sections, 1):
        ...
        if month_display:
            display_lines.append(f"   📊 {', '.join(month_display[:4])}" + ("..." if len(month_display) > 4 else ""))
    display_lines.append("")
    ...
    for section in sections:
        ...
        months_sorted = sorted(section['data'].items(), key=lambda x: int(x[0].replace('ماه ', '')))
...
@flask_app.route('/')
def home():
    return "🤖 بات تحلیلگر اینستاگرام در حال اجراست! (با Render.com)"

# --- شروع برنامه ---
if __name__ == '__main__':
    # شروع بات تلگرام در یک thread جداگانه
